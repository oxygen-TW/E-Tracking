...
class ECTracker():
    def __init__(self, autoVerify=False):
    ...
    def get_resource(self):
        ...
        __VIEWSTATE = body.find(
            'input', {'id': '__VIEWSTATE'}).get('value', None)
        ...
        self.codeImg(
            f'{self.api}/ValidateImage.aspx?ts={datetime.now().strftime("%H%M%S%m%d")}', headers)
        return resource
    ...
    def tracker(self, txtProductNum):
        ...
        with requests.post('https://eservice.7-11.com.tw/E-Tracking/search.aspx', headers=resource['headers'], data=payload, allow_redirects=False) as response:
            ...
            if (body.find('input', {'id': 'txtPage'})['value']) == '2':
                ...
                # 取貨門市
                store_name = pickup_info.find(
                    'span', {'id': 'store_name'}).text
                # 取貨門市地址
                store_address = pickup_info.find(
                    'p', {'id': 'store_address'}).text
                # 取貨截止日
                pickup_deadline = pickup_info.find(
                ...
                for element in shipping.find_all('li'):
                    status_date = re.findall(
                        r"\d{4}/\d{2}/\d{2} \d{2}:\d{2}", element.text)[0]
